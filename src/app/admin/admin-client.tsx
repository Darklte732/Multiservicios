...
export default function AdminClient({ initial }: { initial: AdminInitial }) {
    ...
    startTransition(async () => {
      try {
        ...
        if (!data?.error) {
          ...
          setVersions((data.versions as PromptVersion[]) || [])
  ...
  const loadImprovement = () => {
    ...
    startTransition(async () => {
      try {
        const data = await fetchImprovement()
        if (!data?.error) {
          setImprovement(data as unknown as ImprovementData)
          setNewPrompt('')
        } else {
  ...
  const applyPrompt = () => {
    ...
    startTransition(async () => {
      try {
        const data = await applyPromptAction(newPrompt, 'Mejora basada en análisis de llamadas')
        if (data?.success) {
          setApplySuccess(true)
          setTimeout(() => setApplySuccess(false), 3000)
          setNewPrompt('')
        } else if (data?.error) {
          console.error('Error applying prompt:', data.error)
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-navy-950 text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        {initial.error && (
          <div className="bg-red-900/30 border border-red-500/40 rounded-xl p-4 text-sm text-red-300">
            {initial.error}
          </div>
        )}
